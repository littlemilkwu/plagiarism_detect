...
def get_code_embedding(ls, model):
    ls_py_txt = []
    for student in ls:
        ls_py_txt.append(student['py_txt'])

    encoder_input = model.tokenize(ls_py_txt, mode='<encoder-only>', padding=True, max_length=512)
    encoder_input = torch.tensor(encoder_input)
    _, embeddings = model(encoder_input.to(DEVICE))
    print('embedding shape:' ,embeddings.shape)
    embeddings = embeddings.detach().cpu()
    return embeddings

def calc_similarity(eb1, eb2):
    eb1 = eb1.unsqueeze(dim=0)
    eb2 = eb2.unsqueeze(dim=0)
    eb1 = F.normalize(eb1, p=2, dim=1)
    eb2 = F.normalize(eb2, p=2, dim=1)
    sim = torch.einsum("ac,bc->ab",eb1, eb2)
    return sim
    ...
    for i in tqdm(range(len(ls_student) - 1), desc='[Calculate Students Similarity]'):
        for j in range(i + 1, len(ls_student)):
            ls_sim_pair.append([
                ...
                calc_similarity(ls_s_embedding[i], ls_s_embedding[j]).item()])
...
def detect_sim_with_chatgpt(ls_student, ls_s_embedding, ls_gpt, ls_gpt_embedding):
    ...
    for i in tqdm(range(len(ls_student)), desc='[Calculate ChatGPT Similarity]'):
        ...
        for j in range(len(ls_gpt)):
            ls_tmp.append(float(calc_similarity(ls_s_embedding[i], ls_s_embedding[j]).item()))
...
def main():
    ...
    ls_s_embedding = get_code_embedding(ls_student, model)
    ls_gpt_embedding = get_code_embedding(ls_gpt, model)
